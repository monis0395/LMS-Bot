...
mybot.loginWithToken(auth[2].token);
...
mybot.on("message", function(message) {
    ...
    if (mcontent.startsWith("!JOIN ")) {
        ...
        if (false) {
            ...
            var fight = "\nType `!FIGHT [MONSTER NAME]` to fight a monster. Type `!Monsters` for monsters list"
    ...
    if (mcontent.startsWith("!FIGHT ")) {
        ...
        if (adventures.hasOwnProperty(user)) {
            adventures[user].stats.hp = adventures[user].stats.maxhp;
            saveAdventures();

            if (adventures[user].name === monster) {
    ...
    if (mcontent.startsWith("!COMBO ")) {
        ...
        if (Object.keys(adventures).indexOf(user) <= -1 || adventures[user].stats.hp <= 0) {
        ...
        for (var i = 0; i < combo.length; i++) {
            ...
            if (missHit > 10) {
                ...
                userloose = Math.floor(userloose);
                totalMobDmg += userloose;
                msg += "\n- Missed You loose " + userloose + "HP";
                users[user].hp -= userloose;
                saveUsers();
            }
        }
        console.dir("Mob Hp: " + Math.floor(adventures[user].stats.hp));
        msg += "\n+ Total Damage Dealt: " + totalDamg;
        msg += "\n- Total Damage Received: " + totalMobDmg;
        msg += "\n+ Heatlh: " + Math.floor(users[user].hp) + "/" + users[user].maxhp + "HP";
        msg += " Mana: " + Math.floor(users[user].mp) + "/" + users[user].maxmp + "MP";
        if (Math.floor(adventures[user].stats.hp) <= 0) {
            ...
            for (var i = oldlvl; i < users[user].level; i++)
                msg += "\nYou have leveled UP!";
        } else if (Math.floor(users[user].hp) <= 0) {
    ...
    if(mcontent === "!BSTATS"){
        try{
            ...
            msg += "+ There's been "+commandUsage+" commands used.\n";
            msg += "+ Globally, "+Object.keys(users).length+" users have started their adventure.\n";
            msg += "+ There's "+Object.keys(adventures).length+" adventures right now.";
